...
public class Changepass extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            if (oldpass.equals(oldpassword) && oldpass != null) {
                if (confirm.equals(newpass) && newpass != null) {
                    boolean passwordChanged = dao.ChangePassword(userID, newpass);
                    if (passwordChanged) {
                        User user = dao.getAccountByID(Integer.parseInt(userID));
                        request.getSession().setAttribute("user", user);
                        response.sendRedirect("homepage");
                        return;
                    } else {
                        // Trường hợp đổi mật khẩu không thành công
                ...
                request.setAttribute("mess", "Your current password is wrong!!");
            }
        } catch (Exception e) {
        }
